...
def send_slack_message(webhook_url, message, color='#6d6c6c'):
...
def format_exception_message(c7n_message, exception):
    ...
    slack_message_template = jinja_env.get_template('exception')
...
def lambda_handler(event, context):
    ...
    # Yes this is broad but we want to send info on any exception to slack
        ...
        send_slack_message(webhook_url, slack_message, color='#0f0f0f')
        raise
